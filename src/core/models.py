...
class Server(Base):
    ...
    def is_available(self) -> bool:
        return self.status == ServerStatus.ONLINE and self.utilization_percent < 90
...
class VPNSession(Base):
    ...
    def total_bytes(self) -> int:
        return int(self.bytes_sent + self.bytes_received)
...
class AuditLog(Base):
    ...
    # Additional data (SQLAlchemy reserves attribute name "metadata")
    meta = Column("metadata", JSON, default=dict)
